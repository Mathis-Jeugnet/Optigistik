...
export default function TrafficIncidentsPanel() {
  ...
  // Par défaut, l'incident dure 2 heures
  ...
  const [incidentEndTime, setIncidentEndTime] = useState(endTimeDefault) // NOUVEAU
  
  const [isSearching, setIsSearching] = useState(false)
  const [resolvedGeo, setResolvedGeo] = useState<{lat: number, lng: number} | null>(null) // NOUVEAU
  ...
  // NOUVEAU : Étape 1 - Vérifier l'adresse
  const handleVerifyAddress = async () => {
    ...
    try {
      ...
      if (!geo) {
        alert("Impossible de localiser cette adresse. Essayez d'être plus précis (ex: ajouter la ville).");
      } else {
  ...
  // Étape 2 - Enregistrer l'incident
  const handleAddIncident = async () => {
    ...
    try {
      await addGlobalIncident({
        ...
        date: incidentDate,
        time: incidentTime,
        endTime: incidentEndTime // NOUVEAU
      })
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input
              ...
              required
              value={addressInput}
              // Si l'utilisateur modifie le texte, on annule la validation
              onChange={(e) => { setAddressInput(e.target.value); setResolvedGeo(null); }}
              disabled={isSearching}
              placeholder="Ex: Pont de la Guillotière, Lyon"
              className="flex-1 px-4 py-3 text-sm text-opti-blue border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-opti-red/20 focus:border-opti-red disabled:bg-slate-50 transition-colors"
            />
            {!resolvedGeo && (
              <button
                type="button"
                ...
                className="px-4 py-3 bg-slate-100 hover:bg-slate-200 text-opti-blue font-bold rounded-2xl transition-colors disabled:opacity-50"
              >
        ...
        {resolvedGeo && (
          <div className="bg-emerald-50 text-emerald-700 text-xs font-bold p-3 rounded-xl border border-emerald-100 flex items-center gap-2">
            ...
            Localisation confirmée (Lat: {resolvedGeo.lat.toFixed(4)}, Lng: {resolvedGeo.lng.toFixed(4)})
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-xs font-bold text-opti-blue uppercase tracking-wide mb-2 flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5" /> Fin est.
            </label>
            <input
              type="time"
                ...
                <div className="flex items-start gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500 font-medium mt-0.5 flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {inc.time} à {inc.endTime} • {inc.radiusInMeters}m
                    </p>
                  </div>
                </div>
                <button
